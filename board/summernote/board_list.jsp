        ...
        <input type="hidden" value="${list_search}" name="list_search">
            ...
            <div class="board_top">
                ...
                <c:if test="${sessionScope.grade =='a' || sessionScope.grade =='s'}">
                	...
                	<button type="submit" class="del_btn" id="del_btn" formaction="/admin/board_del" onsubmit="deleteCheck()" disabled>선택 삭제</button>
                	<span class="del_count">(0)</span></span>
                	<input type="button" value="게시물 등록" class="add_contents" onclick="location.href='/admin/board_write';">
                </c:if>
            </div>
            <!-- board column title -->
            ...
            <c:if test="${sessionScope.grade == 'a' || sessionScope.grade == 's'}">
                		<button  class="event_end_button" formaction="/admin/event_end">
                			선택 이벤트 종료
                		</button>
	        </c:if>
        </form>
			...
			<form method="get" name="sort_search" action="board_list">
	                <!-- sort -->
